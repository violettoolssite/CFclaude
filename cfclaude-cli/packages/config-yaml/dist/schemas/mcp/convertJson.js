 ...
 */
export function convertJsonEnvToYamlEnv(env) {
    if (!env)
        return undefined;
 ...
 */
export function convertYamlEnvToJsonEnv(env) {
    if (!env)
        return undefined;
    return Object.fromEntries(Object.entries(env).map(([key, value]) => [
        key,
 ...
 */
export function convertJsonMcpConfigToYamlMcpConfig(name, jsonConfig) {
    const warnings = [];
    // STDIO
    if ("command" in jsonConfig) {
        if (jsonConfig.envFile) {
            warnings.push(`envFile is not supported in Continue MCP configuration (server "${name}"). Environment variables from file will not be used.`);
        }
        const stdioConfig = {
            name,
            type: "stdio",
 ...
 */
export function convertYamlMcpConfigToJsonMcpConfig(yamlConfig) {
    const { name, faviconUrl } = yamlConfig;
    const warnings = [];
    if (faviconUrl) {
        warnings.push(`\`faviconUrl\` from YAML MCP config not supported in Claude-style JSON, will be removed from server ${name}`);
    }
    ...
    if ("command" in yamlConfig) {
        ...
        if (cwd) {
            warnings.push(`\`cwd\` from YAML MCP config not supported in Claude-style JSON, will be removed from server ${name}`);
        }
        return {
            name,
            MCP_TIMEOUT,
    ...
    if ("url" in yamlConfig) {
        ...
        for (const key of Object.keys(unsupportedReqOptions)) {
            warnings.push(`${key} requestOption from YAML MCP config not supported in Claude-style JSON, will be ignored in server ${name}`);
        }
        const httpOrSseJsonConfig = {
            url,
            headers,
        };
        if (yamlConfig.type) {
...
export function converMcpServersJsonConfigFileToYamlBlocks(jsonFile) {
    const allWarnings = [];
    const jsonEntries = Object.entries(jsonFile.mcpServers ?? {});
    const yamlConfigs = jsonEntries.map(([name, config]) => {
        const { warnings, yamlConfig } = convertJsonMcpConfigToYamlMcpConfig(name, config);
