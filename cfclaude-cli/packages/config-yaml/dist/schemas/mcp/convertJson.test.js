import { converMcpServersJsonConfigFileToYamlBlocks, convertJsonMcpConfigToYamlMcpConfig, convertYamlMcpConfigToJsonMcpConfig, } from "./convertJson.js";
import { claudeDesktopLikeConfigFileSchema, mcpServersJsonSchema, } from "./json.js";
describe("convertJsonMcpConfigToYamlMcpConfig", () => {
    describe("STDIO configurations", () => {
        test("converts basic stdio config", () => {
            ...
            const result = convertJsonMcpConfigToYamlMcpConfig("test-server", jsonConfig);
        ...
        test("converts stdio config with all fields", () => {
            ...
            const result = convertJsonMcpConfigToYamlMcpConfig("python-server", jsonConfig);
            expect(result.yamlConfig).toEqual({
                name: "python-server",
                type: "stdio",
                command: "python",
        ...
        test("warns about unsupported envFile", () => {
            ...
            const result = convertJsonMcpConfigToYamlMcpConfig("env-server", jsonConfig);
            expect(result.yamlConfig).toEqual({
                name: "env-server",
                type: "stdio",
                command: "node",
    ...
    describe("SSE/HTTP configurations", () => {
        test("converts basic SSE config", () => {
            ...
            const result = convertJsonMcpConfigToYamlMcpConfig("sse-server", jsonConfig);
            expect(result.yamlConfig).toEqual({
                name: "sse-server",
                url: "https://api.example.com/sse",
            });
        ...
        test("converts SSE config with type and headers", () => {
            ...
            const result = convertJsonMcpConfigToYamlMcpConfig("sse-auth", jsonConfig);
            expect(result.yamlConfig).toEqual({
                name: "sse-auth",
                type: "sse",
                url: "https://api.example.com/sse",
        ...
        test("converts HTTP config", () => {
            ...
            const result = convertJsonMcpConfigToYamlMcpConfig("http-server", jsonConfig);
            expect(result.yamlConfig).toEqual({
                name: "http-server",
    ...
    test("throws error for invalid config", () => {
        ...
        expect(() => convertJsonMcpConfigToYamlMcpConfig("invalid", invalidConfig)).toThrowError("Invalid MCP server configuration");
    });
});
describe("convertYamlMcpConfigToJsonMcpConfig", () => {
    describe("STDIO configurations", () => {
        ...
        test("warns about unsupported cwd field", () => {
            ...
            expect(result.warnings).toHaveLength(1);
            expect(result.warnings[0]).toBe("`cwd` from YAML MCP config not supported in Claude-style JSON, will be removed from server cwd-server");
        });
        test("warns about unsupported faviconUrl", () => {
            ...
            const result = convertYamlMcpConfigToJsonMcpConfig(yamlConfig);
            expect(result.warnings).toHaveLength(1);
            expect(result.warnings[0]).toBe("`faviconUrl` from YAML MCP config not supported in Claude-style JSON, will be removed from server icon-server");
        });
    ...
    describe("SSE/HTTP configurations", () => {
        ...
        test("warns about unsupported requestOptions fields", () => {
            ...
            expect(result.warnings).toContain("timeout requestOption from YAML MCP config not supported in Claude-style JSON, will be ignored in server complex-server");
            expect(result.warnings).toContain("proxy requestOption from YAML MCP config not supported in Claude-style JSON, will be ignored in server complex-server");
            expect(result.warnings).toContain("verifySsl requestOption from YAML MCP config not supported in Claude-style JSON, will be ignored in server complex-server");
        });
    ...
    test("throws error for invalid config", () => {
        ...
        expect(() => convertYamlMcpConfigToJsonMcpConfig(invalidConfig)).toThrowError("Invalid MCP server configuration");
    });
});
describe("converMcpServersJsonConfigFileToYamlBlocks", () => {
    ...
    test("converts parsed JSON file", () => {
        ...
        const parsed = claudeDesktopLikeConfigFileSchema.parse(JSON.parse(jsonString));
    ...
    test("handles mixed valid and problematic configurations", () => {
        ...
        expect(result.yamlConfigs.find((c) => c.name === "good-server")).toBeTruthy();
        expect(result.yamlConfigs.find((c) => c.name === "warning-server")).toBeTruthy();
        // Check that the warning server still has its env vars
        const warningServer = result.yamlConfigs.find((c) => c.name === "warning-server");
...
describe("Environment variable conversion", () => {
    describe("JSON to YAML conversion", () => {
        ...
        test("handles multiple variables in one value", () => {
            const jsonConfig = {
                command: "node",
                env: {
                    CONNECTION_STRING: "postgres://${DB_USER}:${DB_PASS}@${DB_HOST}:5432/mydb",
            ...
            expect(result.yamlConfig).toEqual({
                ...
                command: "node",
                env: {
                    CONNECTION_STRING: "postgres://${{ secrets.DB_USER }}:${{ secrets.DB_PASS }}@${{ secrets.DB_HOST }}:5432/mydb",
                    API_URL: "https://${{ secrets.API_HOST }}/v1/${{ secrets.API_VERSION }}",
                },
            });
        });
    ...
    describe("YAML to JSON conversion", () => {
        ...
        test("handles multiple variables in one value", () => {
            const yamlConfig = {
                ...
                env: {
                    CONNECTION_STRING: "postgres://${{ secrets.DB_USER }}:${{ secrets.DB_PASS }}@${{ secrets.DB_HOST }}:5432/mydb",
                    API_URL: "https://${{ inputs.API_HOST }}/v1/${{ secrets.API_VERSION }}",
                },
            };
            const result = convertYamlMcpConfigToJsonMcpConfig(yamlConfig);
            expect(result.jsonConfig).toEqual({
    ...
    describe("Roundtrip conversion", () => {
        test("JSON -> YAML -> JSON preserves values", () => {
            ...
            // Convert to YAML
            const yamlResult = convertJsonMcpConfigToYamlMcpConfig("test", originalJson);
            // Convert back to JSON
            const jsonResult = convertYamlMcpConfigToJsonMcpConfig(yamlResult.yamlConfig);
...
//# sourceMappingURL=convertJson.test.js.map