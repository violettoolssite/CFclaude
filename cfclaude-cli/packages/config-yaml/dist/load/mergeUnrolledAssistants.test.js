...
describe("mergeUnrolledAssistants", () => {
    ...
    it("should merge all block types without duplicates", () => {
        ...
        expect(result.rules?.map((r) => (typeof r === "string" ? r : r?.name))).toEqual(["code-formatting", "typescript-style"]);
    ...
    it("should deduplicate rules by name", () => {
        ...
        expect(result.rules).toHaveLength(2);
        expect(result.rules?.map((r) => (typeof r === "string" ? r : r?.name))).toEqual(["typescript-style", "naming-convention"]);
        // Should keep the incoming version of typescript-style (base version filtered as duplicate)
        const typescriptRule = result.rules?.[0];
    ...
    it("should deduplicate docs by name", () => {
        ...
        expect(result.docs?.[0]?.startUrl).toBe("https://different-api.example.com/docs");
    });
...
//# sourceMappingURL=mergeUnrolledAssistants.test.js.map