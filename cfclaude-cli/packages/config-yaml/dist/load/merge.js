import { BLOCK_TYPES } from "../browser.js";
...
export function mergePackages(current, incoming) {
    return {
        ...
        requestOptions: mergeConfigYamlRequestOptions(current.requestOptions, incoming.requestOptions),
    };
}
export function mergeUnrolledAssistants(current, incoming) {
    const assistant = {
        ...current,
        env: { ...current.env, ...incoming.env },
        requestOptions: mergeConfigYamlRequestOptions(current.requestOptions, incoming.requestOptions),
    ...
    for (const blockType of BLOCK_TYPES) {
        ...
        if (deduplicated.length > 0) {
            assistant[blockType] = deduplicated;
        }
        else {
...
export function mergeConfigYamlRequestOptions(base, global) {
    if (!base && !global) {
        return undefined;
    }
    if (!base) {
