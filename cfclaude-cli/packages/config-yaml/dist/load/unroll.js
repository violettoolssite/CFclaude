...
import { mergeConfigYamlRequestOptions } from "../browser.js";
import { encodeSecretLocation } from "../interfaces/SecretResult.js";
import { decodeFQSN, decodePackageIdentifier, encodeFQSN, encodePackageIdentifier, encodePackageSlug, packageSlugsEqual, } from "../interfaces/slugs.js";
...
export function parseConfigYaml(configYaml) {
    try {
        ...
        if (e instanceof Error &&
            ...
            throw new Error(`Failed to parse config: ${e instanceof Error ? e.message : e}`);
        }
    }
}
...
export function replaceInputsWithSecrets(yamlContent) {
    const inputsToSecretsMap = {};
...
export async function unrollAssistantFromContent(id, rawYaml, registry, options) {
    ...
    const { config: unrolledAssistant, configLoadInterrupted, errors, } = await unrollBlocks(parsedYaml, registry, options.injectBlocks, options.allowlistedBlocks, options.blocklistedBlocks, options.injectRequestOptions);
    ...
    const renderedConfig = useProxyForUnrenderedSecrets(parseAssistantUnrolled(renderedYaml), id, options.orgScopeId, options.onPremProxyUrl);
...
export async function unrollBlocks(assistant, registry, injectBlocks, allowlistedBlocks, blocklistedBlocks, injectRequestOptions) {
    const errors = [];
    ...
    if (injectRequestOptions) {
        unrolledAssistant.requestOptions = mergeConfigYamlRequestOptions(assistant.requestOptions, injectRequestOptions);
    }
    else {
    ...
    const sectionPromises = sections.map(async (section) => {
        ...
        const blockPromises = assistant[section].map(async (unrolledBlock, index) => {
            // "uses/with" block
            if ("uses" in unrolledBlock) {
                try {
                    ...
                    if (!isPackageAllowed(blockIdentifier, allowlistedBlocks, blocklistedBlocks)) {
                        throw new Error(`${blockIdentifier.uriType === "slug"
                            ? encodePackageSlug({
                                ownerSlug: blockIdentifier.fullSlug.ownerSlug,
                                packageSlug: blockIdentifier.fullSlug.packageSlug,
                            })
                            : encodePackageIdentifier(blockIdentifier)} is block listed and can not be used.`);
                    }
                    const blockConfigYaml = await resolveBlock(blockIdentifier, unrolledBlock.with, registry);
        ...
        ? (async () => {
            const injectedBlockPromises = injectBlocks.map(async (injectBlock) => {
                try {
                    ...
                    const blockConfigYamlWithSecrets = replaceInputsWithSecrets(blockConfigYaml);
                    const blockConfigYamlWithFQSNs = renderTemplateData(blockConfigYamlWithSecrets, {
                        secrets: extractFQSNMap(blockConfigYamlWithSecrets, [
                            injectBlock,
                        ]),
                    });
                    const resolvedBlock = parseMarkdownRuleOrConfigYaml(blockConfigYamlWithFQSNs, injectBlock);
    ...
    for (const sectionResult of sectionResults) {
        if (sectionResult.blocks) {
            unrolledAssistant[sectionResult.section] = sectionResult.blocks.filter((block) => !detector.isDuplicated(block, sectionResult.section));
        }
    }
    // Assign rules result
    if (rulesResult.rules) {
        unrolledAssistant.rules = rulesResult.rules.filter((rule) => !detector.isDuplicated(rule, "rules"));
    }
    // Add injected blocks
    for (const { blockType, resolvedBlock, source, } of injectedResult.injectedBlocks) {
        const key = blockType;
        if (!unrolledAssistant[key]) {
            unrolledAssistant[key] = [];
        }
        const filteredBlocks = injectLocalSourceFile(key, resolvedBlock, source).filter((block) => !detector.isDuplicated(block, blockType));
...
function injectLocalSourceFile(blockType, resolvedBlock, source) {
    const blocks = resolvedBlock[blockType] ?? [];
    if (source === undefined) {
...
export function parseMarkdownRuleOrAssistantUnrolled(content, id) {
...
function inputsToFQSNs(inputs, blockIdentifier) {
    ...
    for (const [key, value] of Object.entries(inputs)) {
        ...
        if (value === undefined || value === null || typeof value !== "string") {
            console.warn(`Skipping input "${key}" with invalid value type: ${typeof value}. Expected string.`);
            continue;
        }
