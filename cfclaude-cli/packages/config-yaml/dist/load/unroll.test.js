import { fillTemplateVariables, getTemplateVariables, parseMarkdownRuleOrAssistantUnrolled, replaceInputsWithSecrets, } from "./unroll.js";
...
This is the rule 
`;
        const result = parseMarkdownRuleOrAssistantUnrolled(markdownContent, mockId);
...
model: # should be models
  - name: model
    ...
    provider: ollama
`;
        expect(() => parseMarkdownRuleOrAssistantUnrolled(invalidContent, mockId)).toThrow();
    });
...
describe("getTemplateVariables edge cases", () => {
    it("handles undefined input gracefully", () => {
        const result = getTemplateVariables(undefined);
        expect(result).toEqual([]);
    });
    it("handles null input gracefully", () => {
        const result = getTemplateVariables(null);
        expect(result).toEqual([]);
    });
    it("handles non-string input gracefully", () => {
        const result = getTemplateVariables(123);
        expect(result).toEqual([]);
    });
    ...
    it("extracts multiple template variables", () => {
        const result = getTemplateVariables("\${{ secrets.key1 }} and \${{ inputs.key2 }}");
...
describe("fillTemplateVariables edge cases", () => {
    it("handles undefined input gracefully", () => {
        const result = fillTemplateVariables(undefined, {});
        expect(result).toBe("");
    });
    it("handles null input gracefully", () => {
        const result = fillTemplateVariables(null, {});
        expect(result).toBe("");
    });
    it("handles non-string input gracefully", () => {
        const result = fillTemplateVariables(123, {});
        expect(result).toBe("");
    });
    it("handles empty string correctly", () => {
...
//# sourceMappingURL=unroll.test.js.map