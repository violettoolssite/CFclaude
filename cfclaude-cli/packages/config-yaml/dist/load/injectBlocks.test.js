...
`;
        const injectedBlockId = {
            uriType: "file",
            fileUri: "file:///test/openai-block.yaml",
        };
        mockRegistry.setContent("file:///test/openai-block.yaml", injectedBlockContent);
        // Unroll with injected blocks
        const result = await unrollBlocks(assistant, mockRegistry, [injectedBlockId], // inject the block
...
`;
        ...
        mockRegistry.setContent("file:///test/rules-block.yaml", injectedBlockContent);
        const result = await unrollBlocks(assistant, mockRegistry, [injectedBlockId], undefined, undefined, undefined);
        expect(result.config).toBeDefined();
        ...
        if (typeof rule === "string") {
            ...
            expect(rule.rule).toBe("Use ${{ secrets.//codeStyle }} formatting consistently");
        }
        // Check that no input variables remain
        const configStr = JSON.stringify(result.config);
...
`;
        ...
        mockRegistry.setContent("file:///test/docs-block.yaml", injectedBlockContent);
        const result = await unrollBlocks(assistant, mockRegistry, [injectedBlockId], undefined, undefined, undefined);
        expect(result.config).toBeDefined();
...
`;
        ...
        mockRegistry.setContent("file:///test/prompt-block.yaml", injectedBlockContent);
        const result = await unrollBlocks(assistant, mockRegistry, [injectedBlockId], undefined, undefined, undefined);
        expect(result.config).toBeDefined();
        ...
        expect(prompt.prompt).toBe("You are a ${{ secrets.//roleType }} assistant. Use ${{ secrets.//responseStyle }} responses.");
...
`;
        ...
        const result = await unrollBlocks(assistant, mockRegistry, [modelBlockId, rulesBlockId], undefined, undefined, undefined);
        expect(result.config).toBeDefined();
        ...
        if (typeof rule === "string") {
            ...
            expect(rule.rule).toBe("Follow ${{ secrets.//codingStandard }} conventions");
        }
...
`;
        ...
        mockRegistry.setContent("file:///test/static-block.yaml", staticBlockContent);
        const result = await unrollBlocks(assistant, mockRegistry, [blockId], undefined, undefined, undefined);
        expect(result.config).toBeDefined();
...
`;
        ...
        expect(ruleText).toBe("Input: ${{ secrets.//userSetting }} | Secret: ${{ secrets.//apiKey }} | Continue: ${{ continue.workspaceRoot }}");
...
`;
        ...
        mockRegistry.setContent("file:///test/nested-block.yaml", nestedBlockContent);
        const result = await unrollBlocks(assistant, mockRegistry, [blockId], undefined, undefined, undefined);
        expect(result.config).toBeDefined();
        expect(result.config.rules).toBeDefined();
        expect(result.config.rules.length).toBe(1);
        ...
        expect(ruleText).toBe("Database config: host=${{ secrets.//db.host }};port=${{ secrets.//db.port }};user=${{ secrets.//db.user }}");
...
`;
        ...
        const model = result.config.models[0];
        expect(model.apiKey).toBe("${{ secrets.anthropic/claude-sonnet/ANTHROPIC_API_KEY }}");
...
//# sourceMappingURL=injectBlocks.test.js.map