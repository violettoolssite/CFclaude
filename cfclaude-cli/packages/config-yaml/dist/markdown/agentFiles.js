...
export function parseAgentFile(content) {
    ...
    if (!frontmatter.name) {
        throw new Error("Agent file must contain YAML frontmatter with a 'name' field");
    }
 ...
 */
export function serializeAgentFile(agentFile) {
    const { prompt, ...frontmatter } = agentFile;
    // Filter out undefined values from frontmatter
    const cleanFrontmatter = Object.fromEntries(Object.entries(frontmatter).filter(([, value]) => value !== undefined));
 ...
 */
export function parseAgentFileTools(toolsString) {
    if (!toolsString?.trim()) {
    ...
    for (const toolRef of toolRefs) {
        if (toolRef === "built_in") {
        ...
        else if (toolRef.startsWith("http://") ||
            ...
            if (lastColonIndex > protocolEndIndex) {
                ...
                if (/^\d+(?:$|[/?#])/.test(afterLastColon)) {
                    ...
                    if (/\s/.test(toolRef)) {
                        throw new Error(`Invalid MCP tool reference "${toolRef}": colon-separated tool references cannot contain whitespace. ` +
                    ...
                    throw new Error(`Invalid URL-based MCP tool reference "${toolRef}": the part after the last colon must be either a port number or a valid tool name (alphanumeric, underscores, hyphens).`);
                }
            }
            else {
...
export function parseAgentFileRules(rules) {
    return rules
        .split(",")
        .map((r) => r.trim())
        .filter(Boolean);
}
//# sourceMappingURL=agentFiles.js.map