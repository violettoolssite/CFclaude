import { parseAgentFile, parseAgentFileTools, serializeAgentFile, } from "./agentFiles.js";
const example = `
---
...
describe("parseAgentFile", () => {
    ...
    test("throws error when name field is missing", () => {
        expect(() => parseAgentFile(invalidExample)).toThrow("Agent file must contain YAML frontmatter with a 'name' field");
    });
    test("throws error when no frontmatter is present", () => {
        expect(() => parseAgentFile(noFrontmatterExample)).toThrow("Agent file must contain YAML frontmatter with a 'name' field");
    });
...
`.trim();
        ...
        expect(result.prompt).toBe("This has --- in the middle\nAnd more content after");
    });
...
Prompt
`.trim();
        expect(() => parseAgentFile(invalidNameType)).toThrow("Invalid agent file frontmatter");
    });
});
...
Prompt
`.trim();
        expect(() => parseAgentFile(emptyNameExample)).toThrow("Agent file must contain YAML frontmatter with a 'name' field");
    });
...
Prompt
`.trim();
        expect(() => parseAgentFile(nullNameExample)).toThrow("Agent file must contain YAML frontmatter with a 'name' field");
    });
});
describe("parseAgentFileTools", () => {
    ...
    it("should parse specific MCP tools", () => {
        const result = parseAgentFileTools("owner/package:tool1, owner/package:tool2");
        expect(result).toEqual({
            tools: [
    ...
    it("should parse mixed tool types", () => {
        const result = parseAgentFileTools("anmcp/serverslug:a_tool, anmcp/serverslug:another_tool, asecond/mcpserver, bash, read, edit");
        expect(result).toEqual({
            tools: [
    ...
    it("should parse mixed tools with built_in keyword", () => {
        const result = parseAgentFileTools("built_in, anmcp/serverslug:a_tool, asecond/mcpserver");
        expect(result).toEqual({
            tools: [
                { mcpServer: "anmcp/serverslug", toolName: "a_tool" },
                { mcpServer: "asecond/mcpserver" },
    ...
    it("should deduplicate MCP servers", () => {
        const result = parseAgentFileTools("owner/package:tool1, owner/package:tool2, owner/package, other/server");
        expect(result).toEqual({
            tools: [
                { mcpServer: "owner/package", toolName: "tool1" },
                { mcpServer: "owner/package", toolName: "tool2" },
    ...
    it("should handle extra whitespace", () => {
        const result = parseAgentFileTools("  owner/package:tool1  ,   bash  ,  other/server  ");
        expect(result).toEqual({
            tools: [
                { mcpServer: "owner/package", toolName: "tool1" },
                { toolName: "bash" },
    ...
    it("should handle any MCP server slug format", () => {
        ...
        expect(() => parseAgentFileTools("owner/package:tool, invalid/slug/extra:tool")).not.toThrow();
    });
    ...
    it("should handle empty tool names and empty entries", () => {
        const result = parseAgentFileTools("owner/package:tool1, , owner/package:, bash");
        expect(result).toEqual({
            tools: [
                { mcpServer: "owner/package", toolName: "tool1" },
    ...
    describe("URL-based MCP references", () => {
        ...
        it("should parse HTTPS URL with port and tool name", () => {
            const result = parseAgentFileTools("https://mcp.example.com:8080:tool_name");
            expect(result).toEqual({
                tools: [
        ...
        it("should parse URL with path and tool name", () => {
            const result = parseAgentFileTools("https://api.example.com/mcp:tool_name");
            expect(result).toEqual({
                tools: [
        ...
        it("should parse URL with query parameters", () => {
            const result = parseAgentFileTools("https://api.example.com/mcp?key=value");
            expect(result).toEqual({
        ...
        it("should parse URL with query parameters and tool name", () => {
            const result = parseAgentFileTools("https://api.example.com/mcp?key=value:tool_name");
            expect(result).toEqual({
                tools: [
                    {
        ...
        it("should parse tool names with mixed alphanumeric and special chars", () => {
            const result = parseAgentFileTools("https://mcp.example.com:tool_name-123");
            expect(result).toEqual({
                tools: [
        ...
        it("should parse multiple URL-based MCP references", () => {
            const result = parseAgentFileTools("https://mcp1.example.com:tool1, https://mcp2.example.com:tool2");
            expect(result).toEqual({
                tools: [
        ...
        it("should parse mixed URL and slug-based MCP references", () => {
            const result = parseAgentFileTools("https://mcp.example.com:tool1, owner/package:tool2, bash");
            expect(result).toEqual({
                tools: [
        ...
        it("should deduplicate URL-based MCP servers", () => {
            const result = parseAgentFileTools("https://mcp.example.com:tool1, https://mcp.example.com:tool2, https://mcp.example.com");
            expect(result).toEqual({
                tools: [
                    { mcpServer: "https://mcp.example.com", toolName: "tool1" },
        ...
        it("should reject URL with whitespace in tool name", () => {
            expect(() => parseAgentFileTools("https://mcp.example.com:tool name")).toThrow('Invalid URL-based MCP tool reference "https://mcp.example.com:tool name": the part after the last colon must be either a port number or a valid tool name');
        });
        it("should reject URL with invalid characters after colon", () => {
            expect(() => parseAgentFileTools("https://mcp.example.com:invalid@tool")).toThrow('Invalid URL-based MCP tool reference "https://mcp.example.com:invalid@tool": the part after the last colon must be either a port number or a valid tool name');
        });
        it("should reject URL with whitespace before colon", () => {
            expect(() => parseAgentFileTools("https://mcp.example.com :tool")).toThrow('Invalid MCP tool reference "https://mcp.example.com :tool": colon-separated tool references cannot contain whitespace');
        });
        it("should handle URL-based MCP with built_in keyword", () => {
            const result = parseAgentFileTools("built_in, https://mcp.example.com:tool1");
            expect(result).toEqual({
    ...
    describe("whitespace validation in colon-separated MCP tool references", () => {
        it("should reject MCP tool reference with space after colon", () => {
            expect(() => parseAgentFileTools("owner/slug: tool")).toThrow('Invalid MCP tool reference "owner/slug: tool": colon-separated tool references cannot contain whitespace');
        });
        ...
        it("should reject MCP tool reference with space in tool name", () => {
            expect(() => parseAgentFileTools("owner/slug:my tool")).toThrow('Invalid MCP tool reference "owner/slug:my tool": colon-separated tool references cannot contain whitespace');
        });
        ...
        it("should accept valid MCP tool references with hyphens and underscores", () => {
            const result = parseAgentFileTools("owner-name/package_name:tool-name_123");
            expect(result).toEqual({
                tools: [
                    {
        ...
        it("should allow whitespace between comma-separated items", () => {
            const result = parseAgentFileTools("owner/package:tool1,   owner/package:tool2  ,  bash");
            expect(result).toEqual({
                tools: [
                    { mcpServer: "owner/package", toolName: "tool1" },
                    { mcpServer: "owner/package", toolName: "tool2" },
        ...
        it("should reject whitespace in one reference but accept valid references in same string", () => {
            expect(() => parseAgentFileTools("owner/package:tool1, owner/package: tool2, bash")).toThrow('Invalid MCP tool reference "owner/package: tool2": colon-separated tool references cannot contain whitespace');
        });
    ...
    describe("edge cases", () => {
        ...
        it("should parse complex real-world example", () => {
            const result = parseAgentFileTools("built_in, linear/mcp, sentry/mcp:read-alerts, https://custom-mcp.internal.com:8443:custom_tool, bash");
            expect(result).toEqual({
                tools: [
...
//# sourceMappingURL=agentFiles.test.js.map