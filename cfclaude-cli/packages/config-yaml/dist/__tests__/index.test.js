...
import { decodeSecretLocation, packageIdentifierToShorthandSlug, resolveFQSN, resolveSecretLocationInProxy, SecretType, unrollAssistant, } from "../index.js";
...
describe("E2E Scenarios", () => {
    ...
    const proxyEnvSecrets = {
        ANTHROPIC_API_KEY: "sk-ant-env",
        GEMINI_API_KEY: "gemini-api-key-env",
    };
    const localUserSecretStore = {
        get: async function (secretName) {
            return userSecrets[secretName];
        },
        set: function (secretName, secretValue) {
            throw new Error("Function not implemented.");
        },
    };
    const platformClient = {
        resolveFQSNs: async function (fqsns) {
            return await Promise.all(fqsns.map((fqsn) => resolveFQSN("test-user", fqsn, platformSecretStore, "test-org")));
        },
    };
    const environmentSecretStore = {
        get: async function (secretName) {
    ...
    const platformSecretStore = {
        getSecretFromSecretLocation: async function (secretLocation) {
            switch (secretLocation.secretType) {
                ...
                case SecretType.FreeTrial:
                    if (secretLocation.blockSlug.ownerSlug === "test-org" &&
    ...
    const registry = {
        getContent: async function (id) {
            ...
            const filePath = id.uriType === "slug"
    ...
    it("should correctly unroll assistant", async () => {
        ...
        expect(geminiModel.apiKeyLocation).toBe(geminiSecretLocation);
        ...
        const anthropicSecretLocation = "models_add_on:test-org/claude35sonnet/ANTHROPIC_API_KEY";
        expect(anthropicModel.apiKeyLocation).toBe(anthropicSecretLocation);
        ...
        expect(config?.docs?.[0]?.startUrl).toBe("https://docs.python.org/release/3.13.1");
        expect(config?.docs?.[0]?.rootUrl).toBe("https://docs.python.org/release/3.13.1");
        ...
        const decodedGeminiSecretLocation = decodeSecretLocation(geminiSecretLocation);
        // With environment
        const antSecretValue = await resolveSecretLocationInProxy(decodedAnthropicSecretLocation, platformSecretStore, environmentSecretStore);
        expect(antSecretValue).toBe("sk-ant-env");
        const geminiSecretValue = await resolveSecretLocationInProxy(decodedGeminiSecretLocation, platformSecretStore, environmentSecretStore);
        expect(geminiSecretValue).toBe("gemini-api-key-env");
        // Without environment
        const antSecretValue2 = await resolveSecretLocationInProxy(decodedAnthropicSecretLocation, platformSecretStore, undefined);
        expect(antSecretValue2).toBe("sk-ant");
        const geminiSecretValue2 = await resolveSecretLocationInProxy(decodedGeminiSecretLocation, platformSecretStore, undefined);
    ...
    it("should correctly unroll assistant with injected blocks", async () => {
        ...
        expect(typeof config?.rules?.[2] !== "string" &&
    ...
    it("should throw when a block is blocklisted", async () => {
        ...
        expect(result.errors?.some((error) => error.message.includes("test-org/gemini is block listed and can not be used."))).toBe(true);
    });
