...
function testConfig(_config) {
    ...
    if (["chat", "summarize", "edit", "apply"].some((role) => config.roles?.includes(role))) {
...
describe("Configuration", () => {
    it("should configure DeepSeek OpenAI client with correct apiBase and apiKey", () => {
        ...
        expect(deepseek.openai.baseURL).toBe(DEEPSEEK_API_BASE);
        expect(deepseek.openai.apiKey).toBe("sk-xxx");
        const deepseek2 = constructLlmApi({
            provider: "deepseek",
            apiKey: "sk-xxx",
            apiBase: "https://api.example.com",
        });
        expect(deepseek2.openai.baseURL).toBe("https://api.example.com");
    });
    it("should configure Inception OpenAI client with correct apiBase and apiKey", () => {
        ...
        expect(inception.openai.baseURL).toBe(INCEPTION_API_BASE);
        expect(inception.openai.apiKey).toBe("sk-xxx");
        const inception2 = constructLlmApi({
            provider: "inception",
            apiKey: "sk-xxx",
            apiBase: "https://api.example.com",
        });
        expect(inception2.openai.baseURL).toBe("https://api.example.com");
    });
    it("should configure Azure OpenAI client with root URL and trailing slash", () => {
        ...
        expect(azure.openai.baseURL).toBe("https://test-azure-openai.azure.com/openai/deployments/gpt-4.1");
        expect(azure.openai.apiKey).toBe("sk-xxx");
    });
    it("should configure Azure OpenAI client with path and trailing slash", () => {
        ...
        // The Azure client modifies the baseURL to include the deployment
        expect(azure.openai.baseURL).toBe("https://test-azure-openai.azure.com/v1/openai/deployments/gpt-4.1");
        expect(azure.openai.apiKey).toBe("sk-xxx");
    });
    ...
    describe("ollama api base", () => {
        it('should have correct default API base for "ollama"', () => {
            ...
            expect(ollama.openai.baseURL).toBe("http://localhost:11434/v1/");
        });
        it('should append /v1 to apiBase for "ollama"', () => {
            ...
            expect(ollama.openai.baseURL).toBe("http://localhost:123/v1/");
        });
        ...
        it("should append v1 if apiBase is like myhostv1/", () => {
            ...
            expect(ollama.openai.baseURL).toBe("https://myhostv1/v1/");
        });
        it("should preserve query params and append v1 in apibase", () => {
            ...
            expect(ollama.openai.baseURL).toBe("https://test.com:123/ollama-server/v1/?x=1");
        });
    });
