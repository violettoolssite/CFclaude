...
dotenv.config();
function testVercelProvider(config, featureFlag) {
...
describe("Contract Compatibility Tests", () => {
    describe("OpenAI: Original vs Vercel SDK", () => {
        ...
        test("streaming output format matches", async () => {
            ...
            console.log(`OpenAI - Original chunks: ${originalChunks.length}, Vercel chunks: ${vercelChunks.length}`);
            expect(vercelChunks.length).toBeGreaterThan(0);
            // Verify chunk structure matches
            const originalContentChunks = originalChunks.filter((c) => c.choices.length > 0);
            const vercelContentChunks = vercelChunks.filter((c) => c.choices.length > 0);
    ...
    describe("Anthropic: Original vs Vercel SDK", () => {
        ...
        test("streaming output format matches", async () => {
            ...
            // Verify both have chunks
            expect(originalChunks.length).toBeGreaterThan(0);
            console.log(`Anthropic - Original chunks: ${originalChunks.length}, Vercel chunks: ${vercelChunks.length}`);
            expect(vercelChunks.length).toBeGreaterThan(0);
            // Verify chunk structure matches
            const originalContentChunks = originalChunks.filter((c) => c.choices.length > 0);
            const vercelContentChunks = vercelChunks.filter((c) => c.choices.length > 0);
...
//# sourceMappingURL=vercel-sdk.test.js.map