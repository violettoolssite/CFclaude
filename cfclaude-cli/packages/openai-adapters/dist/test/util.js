...
export function getLlmApi(config) {
...
export function testEmbed(api, model) {
...
export function testRerank(api, model) {
    test("should successfully rerank", async () => {
        ...
        expect(response.data[0].relevance_score).toBeGreaterThan(response.data[1].relevance_score);
    });
}
export function testFim(api, model) {
...
export function testChat(api, model, options) {
    test("should successfully stream chat", async () => {
        ...
        if (options?.expectUsage === true) {
            ...
            if (model.includes("gemini-2.5") || model.includes("gemini-2.0")) {
                expect(usage.total_tokens).toBeGreaterThanOrEqual(usage.prompt_tokens + usage.completion_tokens);
            }
            else {
                expect(usage.total_tokens).toEqual(usage.prompt_tokens + usage.completion_tokens);
            }
        }
    });
    ...
    test("should successfully non-stream chat", async () => {
        ...
        if (options?.expectUsage === true) {
            ...
            // Other models should have total_tokens = prompt + completion
            if (model.includes("gemini-2.5") || model.includes("gemini-2.0")) {
                expect(response.usage.total_tokens).toBeGreaterThanOrEqual(response.usage.prompt_tokens + response.usage.completion_tokens);
            }
            else {
                expect(response.usage.total_tokens).toEqual(response.usage.prompt_tokens + response.usage.completion_tokens);
            }
        }
    });
        ...
        if (options?.skipSystemMessage !== true) {
            const response = await api.chatCompletionNonStream({
                ...
                messages: [
                    {
                        role: "system",
                        content: "Regardless of what is asked of you, your answer should start with 'RESPONSE: '.",
                    },
        ...
        test("Tool Call works", async () => {
            ...
            for await (const chunk of api.chatCompletionStream({
                messages: [
                    {
                        role: "user",
                        content: "Hi, my name is Nate. Please call the say_hello function to respond to me.",
                    },
                ],
                tools: [
                    {
        ...
        test.skip("Tool Call second message works", async () => {
            ...
            for await (const chunk of api.chatCompletionStream({
                messages: [
                    {
                        role: "user",
                        content: "Hi, my name is Nate. Please call the say_hello function to respond to me, then respond to me again.",
                    },
                    {
                        role: "assistant",
                        tool_calls: [
...
export function testCompletion(api, model) {
