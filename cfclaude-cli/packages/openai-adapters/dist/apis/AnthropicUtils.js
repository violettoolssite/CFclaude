export function getAnthropicErrorMessage(response) {
 ...
 */
export function isAzureAnthropicEndpoint(apiBase) {
    if (!apiBase) {
        return false;
    }
    try {
...
export function getAnthropicHeaders(apiKey, enableCaching, apiBase) {
    ...
    if (isAzure && apiKey.startsWith("sk-ant-")) {
        console.warn("[Continue] Azure endpoint detected but API key appears to be a standard Anthropic key (sk-ant-*). " +
...
export function addCacheControlToLastTwoUserMessages(messages) {
    let userMessages = 0;
...
export function openAiToolChoiceToAnthropicToolChoice(toolChoice) {
    if (!toolChoice) {
        return undefined;
    }
    switch (toolChoice) {
...
export function openaiToolToAnthropicTool(tool) {
...
export function getAnthropicMediaTypeFromDataUrl(dataUrl) {
