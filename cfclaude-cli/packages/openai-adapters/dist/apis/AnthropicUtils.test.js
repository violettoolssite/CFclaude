...
import { getAnthropicHeaders, isAzureAnthropicEndpoint, } from "./AnthropicUtils.js";
describe("isAzureAnthropicEndpoint", () => {
    describe("should return true for Azure endpoints", () => {
        it("detects Azure AI Foundry endpoint", () => {
            expect(isAzureAnthropicEndpoint("https://my-resource.services.ai.azure.com/anthropic")).toBe(true);
        });
        it("detects Azure Cognitive Services endpoint", () => {
            expect(isAzureAnthropicEndpoint("https://my-resource.cognitiveservices.azure.com/anthropic")).toBe(true);
        });
        it("handles case insensitivity", () => {
            expect(isAzureAnthropicEndpoint("https://my-resource.SERVICES.AI.AZURE.COM/anthropic")).toBe(true);
        });
        it("handles mixed case", () => {
            expect(isAzureAnthropicEndpoint("https://My-Resource.Services.AI.Azure.Com/anthropic/v1/messages")).toBe(true);
        });
    });
    ...
    describe("URL parsing security", () => {
        it("rejects Azure pattern in path (not hostname)", () => {
            ...
            expect(isAzureAnthropicEndpoint("https://evil.com/services.ai.azure.com/anthropic")).toBe(false);
        });
        it("rejects Azure pattern as subdomain of non-Azure domain", () => {
            expect(isAzureAnthropicEndpoint("https://services.ai.azure.com.evil.com/anthropic")).toBe(false);
        });
    });
});
describe("getAnthropicHeaders", () => {
    describe("authentication header", () => {
        it("uses x-api-key for standard Anthropic endpoint", () => {
            const headers = getAnthropicHeaders("test-key", false, "https://api.anthropic.com");
            expect(headers["x-api-key"]).toBe("test-key");
            expect(headers["api-key"]).toBeUndefined();
        });
        it("uses api-key for Azure AI Foundry endpoint", () => {
            const headers = getAnthropicHeaders("azure-key", false, "https://my-resource.services.ai.azure.com/anthropic");
            expect(headers["api-key"]).toBe("azure-key");
            expect(headers["x-api-key"]).toBeUndefined();
        });
        it("uses api-key for Azure Cognitive Services endpoint", () => {
            const headers = getAnthropicHeaders("azure-key", false, "https://my-resource.cognitiveservices.azure.com/anthropic");
            expect(headers["api-key"]).toBe("azure-key");
            expect(headers["x-api-key"]).toBeUndefined();
        });
    ...
    describe("caching headers", () => {
        ...
        it("includes caching header for Azure endpoints too", () => {
            const headers = getAnthropicHeaders("azure-key", true, "https://my-resource.services.ai.azure.com/anthropic");
            expect(headers["anthropic-beta"]).toBe("prompt-caching-2024-07-31");
        });
    });
    ...
    describe("key/endpoint mismatch warning", () => {
        it("warns when Azure endpoint is used with Anthropic-style key", () => {
            ...
            getAnthropicHeaders("sk-ant-test-key", false, "https://my-resource.services.ai.azure.com/anthropic");
            expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining("Azure endpoint detected"));
            warnSpy.mockRestore();
        });
        it("does not warn when Azure endpoint is used with Azure key", () => {
            const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => { });
            getAnthropicHeaders("azure-api-key-12345", false, "https://my-resource.services.ai.azure.com/anthropic");
            expect(warnSpy).not.toHaveBeenCalled();
            warnSpy.mockRestore();
        });
        it("does not warn when standard Anthropic endpoint is used with Anthropic key", () => {
            const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => { });
            getAnthropicHeaders("sk-ant-test-key", false, "https://api.anthropic.com");
            expect(warnSpy).not.toHaveBeenCalled();
            warnSpy.mockRestore();
        });
    });
});
//# sourceMappingURL=AnthropicUtils.test.js.map