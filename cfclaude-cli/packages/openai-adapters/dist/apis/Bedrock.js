import { BedrockRuntimeClient, ConversationRole, ConverseStreamCommand, ImageFormat, InvokeModelCommand, } from "@aws-sdk/client-bedrock-runtime";
...
export class BedrockApi {
    ...
    _convertMessages(oaiMessages, availableTools) {
                            ...
                            if (availableTools.has(toolCall.function.name)) {
                                currentBlocks.push({
                                    toolUse: {
                                        ...
                                        input: safeParseArgs(toolCall.function.arguments, `Call: ${toolCall.function.name} ${toolCall.id}`),
                                    },
                                });
    ...
    _convertBody(oaiBody) {
        ...
        const systemMessageText = typeof systemMessage === "string"
            ? systemMessage
            : systemMessage
    ...
    async *chatCompletionStream(body, signal) {
        ...
        try {
            ...
            if (error instanceof Error) {
                if ("code" in error) {
                    throw new Error(`AWS Bedrock stream error (${error.code}): ${error.message}`);
                }
    ...
    async embed(body) {
        ...
        if (body.model.startsWith("cohere")) {
            ...
            embeddings = await Promise.all(texts.map(async (text) => {
                const payload = {
                    inputText: text,
                };
                const output = await this.getInvokeModelResponseBody(body.model, payload);
