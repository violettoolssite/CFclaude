...
import { GoogleAuth, JWT, auth } from "google-auth-library";
...
export class VertexAIApi {
    ...
    convertGeminiBody(oaiBody, isV1API) {
    ...
    async chatCompletionNonStream(body, signal) {
        ...
        switch (vertexProvider) {
            ...
            case "gemini":
                ...
                requestBody = this.convertGeminiBody(body, url.toString().includes("/v1/"));
                break;
            case "mistral":
        ...
        const response = await customFetch(this.config.requestOptions)(url.toString(), {
            method: "POST",
            headers,
    ...
    async *chatCompletionStream(body, signal) {
        ...
        switch (vertexProvider) {
            case "anthropic":
                ...
                const url = this.buildUrl(`publishers/anthropic/models/${body.model}:streamRawPredict`);
            ...
            case "mistral":
                const headers = await this.getAuthHeaders();
                const mistralResponse = await this.mistralInstance.openai.chat.completions.create(this.mistralInstance.modifyChatBody(body), {
                    signal,
                    headers,
                });
    ...
    async embed(body) {
        ...
        const data = await response.json();
        const embeddings = data.predictions.map((prediction) => prediction.embeddings.values);
        return embedding({
            data: embeddings,
            model: body.model,
        });
