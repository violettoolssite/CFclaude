import { CACHING_STRATEGIES, getAvailableStrategies, getStrategyDescription, } from "./AnthropicCachingStrategies.js";
const body_params = {
    model: "claude",
    stream: true,
    max_tokens: 8192,
...
describe("AnthropicCachingStrategies", () => {
    ...
    describe("getStrategyDescription", () => {
        it("should return correct descriptions for all strategies", () => {
            expect(getStrategyDescription("none")).toBe("No caching - baseline for comparison");
            expect(getStrategyDescription("systemOnly")).toBe("Cache only system messages (current implementation)");
            expect(getStrategyDescription("systemAndTools")).toBe("Cache system messages and tool definitions (high impact)");
            expect(getStrategyDescription("optimized")).toBe("Intelligent caching - system, tools, and large content (best performance)");
        });
    });
...
//# sourceMappingURL=AnthropicCachingStrategies.test.js.map