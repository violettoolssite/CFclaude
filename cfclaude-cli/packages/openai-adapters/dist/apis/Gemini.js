...
import { chatChunk, chatChunkFromDelta, customFetch, embedding, usageChatChunk, } from "../util.js";
import { convertOpenAIToolToGeminiFunction, } from "../util/gemini-types.js";
...
export class GeminiApi {
    ...
    constructor(config) {
        ...
        this.genAI = withNativeFetch(() => new GoogleGenAI({
            apiKey: this.config.apiKey,
        }));
    }
    _oaiPartToGeminiPart(part) {
        switch (part.type) {
            case "refusal":
                return {
                    text: part.refusal,
    ...
    _convertBody(oaiBody, isV1API, includeToolCallIds) {
                ...
                return {
                    ...
                    parts: msg.tool_calls.map((toolCall, index) => {
                        if (toolCall.type === "function" && "function" in toolCall) {
                            ...
                            if (index === 0) {
                                const rawSignature = toolCall?.extra_content?.google?.thought_signature;
    ...
    async *processStreamResponse(response, model) {
        let usage = undefined;
    ...
    async generateStream(genAI, model, convertedBody) {
        ...
        return withNativeFetch(() => genAI.models.generateContentStream({
            model,
    ...
    async *chatCompletionStream(body, _signal) {
        const convertedBody = this._convertBody(body, this.apiBase.includes("/v1/"), true);
        const response = await this.generateStream(this.genAI, body.model, convertedBody);
        yield* this.processStreamResponse(response, body.model);
    }
    async *streamWithGenAI(genAI, body) {
        const convertedBody = this._convertBody(body, false, true);
        const response = await this.generateStream(genAI, body.model, convertedBody);
        yield* this.processStreamResponse(response, body.model);
    }
    ...
    async embed(body) {
        ...
        const response = await customFetch(this.config.requestOptions)(new URL(`${body.model}:batchEmbedContents`, this.apiBase), {
            method: "POST",
            body: JSON.stringify({
        ...
        return embedding({
            ...
            data: data.batchEmbedContents.map((embedding) => embedding.values),
        });
    }
    list() {
        throw new Error("Method not implemented.");
