 ...
 */
export function evaluateTerminalCommandSecurity(basePolicy, command) {
    ...
    try {
        ...
        if (commandLines.length > 1) {
            ...
            for (const line of commandLines) {
                ...
                mostRestrictivePolicy = getMostRestrictive(mostRestrictivePolicy, linePolicy);
...
function evaluateTokensSecurity(tokens, basePolicy, originalCommand) {
    ...
    if (hasEmptyStrings) {
        ...
        const hasEmptyQuotes = originalCommand.includes('""') || originalCommand.includes("''");
        ...
        if (!hasEmptyQuotes || hasVariablePatterns) {
            ...
            const policyWithEmpty = evaluateTokens(tokens, basePolicy, originalCommand);
            // 2. Evaluate without empty strings
            const tokensWithoutEmpty = tokens.filter((t) => t !== "");
            const policyWithoutEmpty = tokensWithoutEmpty.length > 0
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (isOperator(token)) {
            ...
            if (currentCommand.length > 0) {
                const commandPolicy = evaluateSingleCommand(currentCommand, originalCommand);
 ...
 */
function getMostRestrictive(...policies) {
...
function evaluatePipeChain(tokens, startIdx) {
    ...
    const nextCommand = [];
    for (let i = nextCommandStart; i < tokens.length && !isOperator(tokens[i]); i++) {
        const token = tokens[i];
        if (isComment(token)) {
            continue;
        }
...
function isCriticalCommand(baseCommand, args) {
    ...
    const hasRf = allTokens.some((arg) => arg === "-rf" ||
        arg === "-fr" ||
    ...
    const hasDangerousPath = allTokens.some((arg) => arg === "/" ||
        arg === "/*" ||
        arg === "~" ||
        arg === "~/*" ||
        arg === "/usr" ||
    ...
    if (baseCommand === "del") {
        ...
        const hasSystemDrive = args.some((arg) => arg.toLowerCase().includes("c:\\") || arg.includes("c:/"));
    ...
    if (baseCommand === "dd") {
        if (args.some((arg) => arg.includes("of=/dev/") ||
    ...
    if (baseCommand === "icacls") {
        if (args.some((arg) => arg.toLowerCase().includes("everyone:f") ||
...
function isHighRiskFileOperation(baseCommand, args) {
    if (baseCommand === "mv" || baseCommand === "cp" || baseCommand === "copy") {
        ...
        if (args.some((arg) => sensitiveLocations.some((loc) => arg.toLowerCase().includes(loc)))) {
            return true;
        }
    }
    return false;
...
function isHighRiskRmCommand(baseCommand, args) {
    if (baseCommand === "rm") {
        ...
        const hasCriticalPath = args.some((arg) => ["/etc/", "/usr/", "/bin/", "/sbin/"].some((path) => arg.includes(path)));
...
//# sourceMappingURL=evaluateTerminalCommandSecurity.js.map