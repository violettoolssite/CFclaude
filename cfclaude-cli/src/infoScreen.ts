...
import { CF_THEME } from "./asciiArt.js";
import {
  isAuthenticated,
...
function getVersionInfo(): string[] {
  ...
  return [
    CF_THEME.primary("CLI Information:"),
    `  Version: ${CF_THEME.secondary(version)}`,
    `  Working Directory: ${CF_THEME.accent(cwd)}`,
  ];
}
...
function getConfigInfo(): string[] {
  ...
  try {
    ...
    if (configState.configPath) {
      lines.push(`  Path: ${CF_THEME.primary(configState.configPath)}`);
    }
...
function getSessionInfo(): string[] {
  ...
  try {
    ...
    lines.push(
      ...
      `  File: ${CF_THEME.primary(sessionFilePath)}`,
    );
  } catch {
...
function getDiagnosticInfo(): string[] {
  ...
  return [
    ...
    `  Path: ${CF_THEME.primary(nodePath)}`,
    `  Invoked: ${CF_THEME.primary(invokedPath)}`,
  ];
}
