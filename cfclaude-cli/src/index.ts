...
program
  .name("cf")
  .description(
    "CF Coder - Cloudflare-powered AI development assistant. Starts an interactive session by default, use -p/--print for non-interactive output.",
  )
...
addCommonOptions(program)
  ...
  .action(async (prompt, options) => {
    ...
    await posthogService.capture("cliCommand", { command: "cf" });
    ...
    if (options.print && !prompt && !options.agent && !options.resume) {
      ...
      safeStderr('  cf -p "please review my current git diff"\n');
      safeStderr('  echo "hello" | cf -p\n');
      safeStderr('  cf -p "analyze the code in src/"\n');
      safeStderr("  cf -p --agent my-org/my-agent\n");
      safeStderr("  cf -p --prompt my-org/my-prompt\n");
      safeStderr("  cf -p --resume\n");
...
addCommonOptions(
  program
    ...
    .description("Launch a remote instance of the cf agent"),
)
  .option(
    "--url <url>",
...
function showDesktopAppPrompt(): void {
  ...
  process.stdout.write(cfOrange.bold("|") + chalk.yellow("  [!] 未检测到配置，请通过以下方式使用:                       ") + cfOrange.bold("|\n"));
  process.stdout.write(cfOrange.bold("|") + chalk.yellow("                                                              ") + cfOrange.bold("|\n"));
  process.stdout.write(cfOrange.bold("|") + chalk.cyan("  [1] 下载桌面应用:                                           ") + cfOrange.bold("|\n"));
  process.stdout.write(cfOrange.bold("|") + chalk.white("      https://github.com/violettoolssite/CFclaude/releases    ") + cfOrange.bold("|\n"));
  process.stdout.write(cfOrange.bold("|") + chalk.yellow("                                                              ") + cfOrange.bold("|\n"));
  process.stdout.write(cfOrange.bold("|") + chalk.cyan("  [2] 或在桌面应用中点击「启动内置 CF Coder」                  ") + cfOrange.bold("|\n"));
  process.stdout.write(cfOrange.bold("|") + chalk.yellow("                                                              ") + cfOrange.bold("|\n"));
  process.stdout.write(cfOrange.bold("+--------------------------------------------------------------+\n"));
