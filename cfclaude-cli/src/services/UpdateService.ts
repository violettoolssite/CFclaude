...
export class UpdateService extends BaseService<UpdateServiceState> {
  constructor() {
    super("update", {
      autoUpdate: false, // CF Coder: 禁用自动更新
      isAutoUpdate: false,
  ...
  async doInitialize(headless?: boolean) {
    ...
    this.setState({
      status: UpdateStatus.IDLE,
      message: `CF Coder v${this.currentState.currentVersion}`,
    });
      ...
      if (
        ...
        this.setState({
          ...
          message: isUpdateAvailable
            ...
            : `CF Coder v${this.currentState.currentVersion}`,
          isUpdateAvailable,
          latestVersion,
        });
      }
  ...
  private restartCLI(): void {
    try {
      ...
      // Halt/clean up parent cf process
      try {
        ...
        process.stdin.pause();
        // console.clear(); // Don't want to clear things that were in console before cf started
      } catch (e) {
        logger.debug("Error cleaning up terminal:", e);
      }

      // Spawn a new detached cf process
