...
import { CF_COLORS, defaultBoxStyles } from "./styles.js";
...
export function SessionSelector({
  ...
  if (sessions.length === 0) {
    return (
      <Box {...defaultBoxStyles(CF_COLORS.orange)}>
        <Text color="yellow">No previous sessions found.</Text>
        <Text color="gray">Start a new conversation with: cf</Text>
  ...
  return (
    <Box flexDirection="row" width={terminalWidth}>
      ...
      <Box {...defaultBoxStyles(CF_COLORS.orange)} width={listWidth}>
        ...
        {displaySessions.map((session, index) => {
          ...
          const color = isSelected ? CF_COLORS.orange : "white";
      ...
      {showPreview && (
        <Box marginLeft={1} flexGrow={1} width="100%">
          {previewSession ? (
            ...
            <Box
              {...defaultBoxStyles(CF_COLORS.orange)}
